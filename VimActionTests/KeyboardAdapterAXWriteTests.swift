...
// MARK: - 창 경계 j/k 위임 예외 (PR-D1b 세션 5)

/// **요소가 곧 문서가 아닌 앱**의 모양 — Notion은 블록마다 별개 AX 요소이고 그 요소가 소프트
/// 랩된 문단 하나라 논리 줄이 **1개**다. 그래서 창이 위/아래 논리 줄 없음을 정확히 증명하는데,
/// 그 판정이 `j`/`k`를 통째로 죽인다.
private let axSingleLine = "abcdefghij"

private let selectDown = (code: Int64(kVK_DownArrow), flags: CGEventFlags.maskShift)
private let selectUp = (code: Int64(kVK_UpArrow), flags: CGEventFlags.maskShift)

/// 무상태 폴백 금지의 **유일한 예외**를 좁게 고정한다 — 예외가 새면 AX가 쓴 범위 위로 무상태
/// 시퀀스가 나가고, 그것이 이 PR 내내 막아 온 파괴 방향 동전 던지기다.
struct KeyboardAdapterVisualBoundaryDelegationTests {
    /// 전진형은 앱이 포커스로 보는 끝(실측 "끝이 포커스")과 우리 포커스가 같아 위임 결과가
    /// 정확하다 — 얻는 것은 이 PR 이전과 같은 **블록 안의 시각 줄** 이동이다.
    @Test("전진형 세션의 창 경계 j/k는 화살표를 위임한다", arguments: [
        (motion: Motion.lineDown, stroke: selectDown), (motion: .lineUp, stroke: selectUp),
    ])
    func forwardSessionDelegatesAtWindowBoundary(
        _ fixture: (motion: Motion, stroke: (code: Int64, flags: CGEventFlags))
    ) {
        nonisolated(unsafe) var writes = 0
        nonisolated(unsafe) var posted: [CGEvent] = []
        let tracker = axSession(anchor: 0, column: 0)
        ...
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: 1), visualAnchor: tracker,
            onWrite: { _ in writes += 1 }, collecting: { posted.append($0) })

        adapter.execute(
            [.extendSelection(fixture.motion)], profile: axProfile, processID: anyPID)

        #expect(writes == 0)
        #expect(downStrokes(posted).map(\.code) == [fixture.stroke.code])
        #expect(downStrokes(posted).allSatisfy { $0.flags == fixture.stroke.flags })
        // **상태는 손대지 않는다** — 착지를 모르니 좁히는 방향만 허용인데 좁힐 것이 없고,
        // 희망 열을 지우면 다음 `j`가 `.unproven`(스킵)이 되어 연속 위임이 끊긴다.
        #expect(tracker.current == before)
        #expect(tracker.sessionPath == .accessibility)
    }

    /// **위임분은 무상태 폴백 경로와 바이트 동일해야 한다** — 상수 하드코딩이 아니라 실제
    /// keyboard 어댑터의 출력과 대조한다(`unprovenDelegationMatchesKeyboard` 선례). 예외가
    /// "이 PR 이전 동작으로의 낙하"라는 주장이 그래야 구조로 유지된다.
    @Test("창 경계 위임은 keyboard 폴백과 바이트 동일하다")
    func boundaryDelegationMatchesKeyboard() {
        nonisolated(unsafe) var delegated: [CGEvent] = []
        makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: 1),
            visualAnchor: axSession(anchor: 0, column: 0), collecting: { delegated.append($0) }
        ).execute([.extendSelection(.lineDown)], profile: axProfile, processID: anyPID)

        // 상태·경로가 없는 세션 = 무상태 폴백 경로 그대로다.
        nonisolated(unsafe) var fallback: [CGEvent] = []
        makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: 1),
            visualAnchor: VisualAnchorTracker(), collecting: { fallback.append($0) }
        ).execute([.extendSelection(.lineDown)], profile: axProfile, processID: anyPID)

        #expect(!fallback.isEmpty)
        #expect(downStrokes(delegated).map(\.code) == downStrokes(fallback).map(\.code))
        #expect(downStrokes(delegated).map(\.flags) == downStrokes(fallback).map(\.flags))
    }

    /// **후진형은 계속 스킵**이다 — 앱이 포커스로 보는 끝이 우리 **앵커** 쪽이라 `Shift-↑`가
    /// 앵커를 움직인다. 예외가 실측이 닫은 자리 밖으로 새지 않는다는 것이 이 테스트다.
    @Test("후진형 세션의 창 경계 j/k는 여전히 스킵이다", arguments: [Motion.lineDown, .lineUp])
    func backwardSessionStillSkips(_ motion: Motion) {
        nonisolated(unsafe) var writes = 0
        nonisolated(unsafe) var posted: [CGEvent] = []
        // 앵커 5, 앱 앵커는 범위의 **오른쪽 끝**(6)이고 포커스는 범위 시작 2다.
        let tracker = axSession(anchor: 5, side: .right, pinnedEnd: 6, column: 2)
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 2, length: 4), visualAnchor: tracker,
            onWrite: { _ in writes += 1 }, collecting: { posted.append($0) })

        adapter.execute([.extendSelection(motion)], profile: axProfile, processID: anyPID)

        #expect(writes == 0)
        #expect(posted.isEmpty, "후진형은 위임 예외 밖이다")
    }

    /// **연속 위임이 이어져야 한다** — 상태 무갱신이 다음 타를 죽이지 않는 것이 예외 설계의
    /// 두 번째 다리다(희망 열이 살아 있어 다음 `j`도 같은 경로로 흐른다).
    @Test("연속 j는 둘 다 위임된다")
    func repeatedDelegationSurvives() {
        nonisolated(unsafe) var posted: [CGEvent] = []
        let tracker = axSession(anchor: 0, column: 0)
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: 1), visualAnchor: tracker,
        ...
        adapter.execute(
            [.extendSelection(.lineDown), .extendSelection(.lineDown)], profile: axProfile,
            processID: anyPID)

        #expect(downStrokes(posted).map(\.code) == [selectDown.code, selectDown.code])
    }

    /// **charwise 한정**이다 — `V` 세션에서 위임하면 결과가 줄 반올림이 아니라 시각 줄 확장이라
    /// linewise 계약 자체가 깨지고, Notion에서는 이미 블록 전체를 잡고 있어 다음 타에 선택이
    /// 소멸한다(실측).
    @Test("V 세션의 창 경계 j는 위임 예외 밖이다")
    func linewiseSessionIsOutsideTheException() {
        nonisolated(unsafe) var writes = 0
        nonisolated(unsafe) var posted: [CGEvent] = []
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: axSingleLine.utf16.count),
            visualAnchor: axSession(anchor: 0, wise: .linewise, column: 0),
            onWrite: { _ in writes += 1 }, collecting: { posted.append($0) })

        adapter.execute([.extendSelection(.lineDown)], profile: axProfile, processID: anyPID)

        #expect(writes == 0)
        #expect(posted.isEmpty)
    }

    /// **범위 무변화는 위임으로 새지 않는다** — 어댑터는 모션을 다시 거르지 않으므로(위임분은
    /// 모든 모션에 대해 만들어진다) 두 케이스를 합치는 순간 줄 끝 `l` 같은 Vim no-op에서
    /// `Shift-→`가 나간다. 즉 이 단언이 `case .invalid`와 `case .noAdjacentLine`을 갈라 두는
    /// 어댑터 쪽 방어선이다(순수 계층 쪽은 `lineBoundarySplitsFromRangeUnchanged`).
    @Test("범위 무변화는 창 경계와 갈려 스킵으로 남는다")
    func rangeUnchangedStillSkips() {
        nonisolated(unsafe) var writes = 0
        nonisolated(unsafe) var posted: [CGEvent] = []
        let count = axSingleLine.utf16.count
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: count),
            visualAnchor: axSession(anchor: 0, column: 0), onWrite: { _ in writes += 1 },
            collecting: { posted.append($0) })
    ...
    /// 재정의 가드가 예외보다 **앞선다** — 사용자가 "이 앱에서 이 키를 쓰지 마라"고 지시한
    /// 모션은 창 경계에서도 나가지 않는다.
    @Test("재정의된 j는 창 경계에서도 위임되지 않는다")
    func overriddenMotionOutranksTheException() {
        nonisolated(unsafe) var posted: [CGEvent] = []
        let profile = ResolvedProfile(
            AppProfile(
                strategy: .accessibility,
                motions: [.lineDown: .strokes([ConfigKeyStroke(.pageDown)])]))
        let adapter = makeAXAdapter(
            axText: focusedText(axSingleLine, caret: 0, length: 1),
            visualAnchor: axSession(anchor: 0, column: 0), collecting: { posted.append($0) })

        adapter.execute([.extendSelection(.lineDown)], profile: profile, processID: anyPID)

        #expect(posted.isEmpty, "재정의 시퀀스도 무상태 시퀀스다 — 답은 정직한 스킵이다")
    }
}
