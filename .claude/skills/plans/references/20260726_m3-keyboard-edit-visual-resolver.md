...
- **생성일**: 2026-07-26
- **갱신일**: 2026-07-27 (단계 1 코드 완료 — 편집 매퍼·어댑터 배선, 결정 4건 기록. 도그푸딩 대기)
...
## 남은 것
- [ ] **단계 1 도그푸딩 (실기기 — 코드는 완료)**: Notion·Chrome·VS Code에서 `x`·`d/c/y`+모션·`dd/cc/yy`·`d/c/y`+`j k G gg`·`diw/ciw/yiw` 확인. **버그 아닌 것**: 마지막 줄 `dd`·`dG`의 빈 줄 1개 잔존, `change` 후 `u` 2회, TextEdit 등 네이티브 앱의 Opt-화살표 편차. 어긋나면 Notion은 이벤트 드랍부터 의심. 미지원(`caw`·`ci"`)은 "아무것도 안 지워진 채 Insert"가 정상 — 로그로 확인.
...
- **단계 4로 넘긴 항목 1건 추가**: 비-QWERTY 레이아웃에서 오퍼레이터 키가 엉뚱한 `Cmd-` 단축키로 나간다 — 게이트 해제 시 안전판 필요 여부 판단.
