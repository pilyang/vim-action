...
- 작업 브랜치 `feat/app-key-translator` (main에서 분기) — 번역기 구현 완료·테스트 그린이나 **미커밋 상태**. 다음 세션은 커밋/PR 여부 확인 후 "탭↔엔진 배선" 착수 (KeyTranslator는 배선에서 `translate` 호출만 하면 되는 상태).
