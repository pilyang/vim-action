...
- **안전장치 브랜치 코드리뷰 (2026-07-18, `feat/app-safety-toggle`, 워크플로 high)**: 구현 커밋 `4b9ab2f`에 대한 리뷰 findings 7건 중 사용자 승인 2건 반영 완료 — ① off 의미론 강화(tapEnable(false)+재활성화 게이트) ② 탈출 옵션 소유 모델 통일 (둘 다 확정 설계에 개정 반영, 단위 테스트 그린). **미결 후속**: ③ ON 복귀 tapEnable 실패 시 status 미반영(UI가 활성으로 거짓 표시 — 최소 반영은 이번 PR, 상태 관리 일원화는 워치독 항목에 포함할 것) ④ tapEnable+검증+로그 3중복 → 워치독 PR에서 공유 헬퍼 수렴 ⑤ `withTemporaryDefaults` 테스트 지원 파일 분리 ⑥ 메뉴바 Toggle 수동 Binding → @Bindable. off 분기 tapEnable(false)는 TEST_HOST 포트 nil이라 단위 테스트 불가 — 실기기 GREEN 체크리스트에서 확인.
