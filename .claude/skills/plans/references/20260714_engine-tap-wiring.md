...
- [x] **탭↔엔진 배선 — 코드·단위 테스트** (2026-07-17, PR #8, 브랜치 `feat/app-engine-tap-wiring`): 확정 설계대로 구현. 테스트 — `EventTapDecisionTests`(키 시퀀스 픽스처, **decision 수준만 검증** — actions 내용은 엔진 테스트 몫, Plan-2 출력 확장과 결합 방지) + 캐싱 적재→무효화→재조회 + QWERTY 판별 헬퍼 공용 추출. 앱 33건·엔진 9건 그린. 리뷰 에이전트 2종: correctness 클린, silent-failure findings 3건 반영 — ① 분산 노티 selector 기반 `.deliverImmediately` 등록(LSUIElement는 사실상 항상 비활성이라 기본 coalesce는 배달 유예 → 캐시 낡음) ② `kTISNotifyEnabledKeyboardInputSourcesChanged` 추가 관찰(무효화 축 정합) ③ `UCKeyTranslate` 실패 시 캐시 폐기(자가 치유). 실기기 GREEN은 남은 것 참조.
...
- `isInterceptionEnabled`는 `EventTapController` 소유(관찰 프로퍼티) + UserDefaults 영속(init 로드, didSet 저장 — `@Observable` 클래스엔 `@AppStorage` 불가). **off = 콜백 최상단에서 전부 통과 + 엔진 `.insert` 리셋(재생성) + 탭 포트 유지 + 워치독 정지.**
- **on 복귀 시 `tapEnable(true)` 1회 선제 호출** — off 동안(워치독 정지 + 콜백 미발화 상태) 시스템이 탭을 비활성화하면 감지 주체가 없으므로, 복귀 시점에 워치독 첫 폴링(≤2초)을 기다리지 않고 즉시 복구한다.
- 메뉴바 Toggle 항목 + off 전용 글리프 (기존 "square.dashed"는 탭 비활성 표시 — off와 구분되는 심볼 선택은 구현 재량).
- Normal 탈출 옵션: SettingsView `@AppStorage`(기본 on) → on=`[.command, .option]` / off=`[]`를 Configuration으로 번역해 엔진 **재생성 주입**. 설정 변경 시 모드가 insert로 리셋되는데, 설정 조작 중이므로 수용 (판단 근거 포함해 "기록" 단계에 기재).
...
- [ ] **탭↔엔진 배선 — 실기기 GREEN + PR #8 머지** (사용자 동반, 코드는 완료 — 완료된 것 참조): Insert 전부 통과 / Esc→Normal 글리프 전환 / Normal 문자키 삼킴 / `hjkl`·`gg` replace 로그 관측 / Cmd+Space 탈출 / **입력 소스 변경 후 번역 반영(캐시 무효화 — 앱 비활성 상태 배달 포함, silent-failure 리뷰의 검증 권고)**. 검증 전 Quit 경로 확인 + TCC 재부여(아래 주의). 체크리스트는 PR #8 본문에도 있음.
- [ ] **안전장치 — 메뉴바 토글 + 설정**: "안전장치" 의미론대로. GREEN: 실기기 — 토글 off에서 Normal 키 전부 통과 + off 글리프, on 복귀 시 Insert부터 재시작 / 탈출 옵션 on: Normal에서 Cmd-Space 후 타이핑 통과(Insert 복귀), off: Normal 유지.
- [ ] **탭 워치독**: "워치독" 규칙대로. GREEN: 실기기 — 디버거 pause로 탭 비활성 유도 → resume 후 ≤2초+α 내 복구 로그 + 가로채기 재개 (SIGSTOP은 콜백 유실로 부적합 — 결정 문서의 검증 노트).
- [ ] **최종 검증**: 엔진 `swift test` + `xcodebuild test -only-testing:VimActionTests` + 위 항목별 실기기 GREEN 체크리스트 통합 재점검.
- [ ] **기록**: decisions 5건(런루프 재검토 결과=메인 유지, modifier 탈출 옵션 — pending 엣지 포함, 토글 의미론+워치독 게이팅, replace-미실행-시-swallow 과도기 규칙 — **릴리스 무로그 삼킴 수용 조건 포함: 디스패처 마일스톤 전 릴리스 배포 금지 (silent-failure 리뷰 advisory)**, KeyTranslator 레이아웃 캐싱+무효화 — deliverImmediately·enabled 노티·실패 시 폐기 포함, CGEvent→Key 번역 방식은 2026-07-16 기록 완료) + architecture 갱신(mode-engine/system-overview/reentrancy-and-safety).
...
- **현재 기준선 (2026-07-17, PR #8 오픈)**: 배선 코드·단위 테스트 완료가 `feat/app-engine-tap-wiring` 브랜치에 있음 — 다음 세션은 실기기 GREEN 체크 → PR #8 머지부터. 안전장치·워치독 항목은 머지 후 이 브랜치 위 상태에서 착수(같은 파일 `EventTapController.swift` 수정이라 머지 전 병렬 착수는 충돌 유발).
