...
- **생성일**: 2026-07-14
- **갱신일**: 2026-07-20 <!-- 워치독 PR #13 squash 머지(bbca660)·최종 검증 완료 — 남은 것은 기록뿐 -->
...
- [x] **탭 워치독** (2026-07-20, PR #13 squash 머지 `bbca660`): bg 폴링 워치독 + 코드리뷰(워크플로 high) 7 findings 반영(off 시리얼 큐 봉인·홉 FIFO+weak self·틱 순수 함수 추출+enableAndCheck 공용화·스톨 게이트+목적 정정·Status.secureInput) — 결정 2건 기록(관련 링크). 실기기 GREEN: 최초 6항목(2026-07-19) + 리뷰 반영분 3건(2026-07-20 — ① SIGSTOP/SIGCONT: CONT 후 0.31초 복구, 콜백 유실 상황 워치독 단독 복구 ② Secure Input: false-failure 없음. 실측 — 이 OS에선 secure input이 tapIsEnabled를 끄지 않아 .secureInput 상태·lock 글리프는 휴면(방어로 유지, CI 커버) ③ 스톨 게이트(lldb 메인 단독 정지 20초×2): 정지 창 내 재활성화 0건(attach 경계 1회만 — 첫 홉 pending 후 작동하는 설계상 경계), 정지 중 키 통과, 재개 후 즉시 복귀. 실측 — 메인 단독 정지는 OS가 탭을 공식 disable하지 않고 이벤트별 타임아웃 통과로 처리, 공식 disable은 완전 정지에서만 관측).
- [x] **최종 검증** (2026-07-20): 엔진 `swift test` 27건 + `xcodebuild test -only-testing:VimActionTests` 47건 그린, 실기기 GREEN 체크리스트 재점검 완료(워치독 항목 참고).
