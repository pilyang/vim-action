...
- **생성일**: 2026-07-14
- **갱신일**: 2026-07-20 <!-- PR #13 리뷰 반영분 실기기 GREEN 3건 확인까지 — 머지만 남음 -->
...
- [ ] **탭 워치독 PR #13 마무리**: 구현·실기기 GREEN·PR 오픈 완료, **코드리뷰(워크플로 high) 7 findings 반영 완료 (2026-07-19, 5커밋: f161a06 off 시리얼 큐 봉인 / 767f2e8 홉 FIFO+weak self / 7093e22 틱 순수 함수 추출+enableAndCheck 공용화 / 62315c3 스톨 게이트+목적 정정 / 0a99588 Status.secureInput)** — 결정 2건 기록됨(아래 링크). **리뷰 반영분 실기기 GREEN 3건 확인 완료 (2026-07-20, 사용자 동반)**: ① SIGSTOP/SIGCONT — CONT 후 0.31초 복구, "시스템이 탭 비활성화" 0건(콜백 유실 상황 워치독 단독 복구), 재개 후 가로채기 재개 ② Secure Input — 활성 8초 동안 .failed 플립·에러 로그 0건(false-failure 제거 목적 달성). **실측: 이 OS에선 secure input이 tapIsEnabled를 끄지 않음**(이벤트만 상류 억제) → .secureInput 상태·lock 글리프는 휴면(탭이 실제로 꺼지는 OS/경로 대비 방어로 유지, CI 테스트 커버) ③ 스톨 게이트(lldb 메인 스레드 단독 정지 20초×2회, 연타 동반) — 정지 창 내 재활성화 0건(attach 경계 1회만 — 게이트는 첫 홉 pending 후 작동하는 설계상 경계), 정지 중 키 통과(사용자 체감: 초반 씹힘 후 즉시 입력, 2초 주기 재정체 없음), 재개 후 즉시 가로채기 복귀. **실측: 메인 단독 정지에서 OS는 탭을 공식 disable하지 않고 이벤트별 타임아웃 통과로 처리**(재개 시 tapDisabledBy* 콜백·워치독 복구 로그 없음 — 공식 disable은 완전 정지에서만 관측). 남음: **머지**.
- [ ] **최종 검증**: 엔진 `swift test` + `xcodebuild test -only-testing:VimActionTests` + 위 항목별 실기기 GREEN 체크리스트 통합 재점검. (2026-07-19 리뷰 반영 시점 기준 앱 47건·엔진 27건 그린.)
- [ ] **기록**: decisions 잔여분(modifier 탈출 옵션 — pending 엣지 포함, 토글 의미론+워치독 게이팅 — **2026-07-18 개정 포함: off=tapEnable(false)+재활성화 게이트, 탈출 옵션 소유 모델 컨트롤러 통일**, replace-미실행-시-swallow 과도기 규칙 — **릴리스 무로그 삼킴 수용 조건 포함: 디스패처 마일스톤 전 릴리스 배포 금지 (silent-failure 리뷰 advisory)**, KeyTranslator 레이아웃 캐싱+무효화 — deliverImmediately·enabled 노티·실패 시 폐기 포함) + architecture 갱신(mode-engine/system-overview — reentrancy-and-safety는 2026-07-19 갱신 완료). 완료된 기록: CGEvent→Key 번역(2026-07-16), 워치독 스톨 게이트·Secure Input 상태(2026-07-19). **런루프 재검토는 "메인 유지" 기록이 아니라 별도 플랜으로 승격됨** — [20260719_dedicated-tap-runloop-thread.md](20260719_dedicated-tap-runloop-thread.md).
...
- decisions: [20260713_tap-reenable-watchdog-polling.md](../../decisions/references/20260713_tap-reenable-watchdog-polling.md), [20260712_main-runloop-tap-attachment.md](../../decisions/references/20260712_main-runloop-tap-attachment.md), [20260712_key-representation-and-fixture-format.md](../../decisions/references/20260712_key-representation-and-fixture-format.md), [20260719_watchdog-stall-gate-post-stall-recovery.md](../../decisions/references/20260719_watchdog-stall-gate-post-stall-recovery.md) (PR #13 리뷰 반영), [20260719_secure-input-status.md](../../decisions/references/20260719_secure-input-status.md) (PR #13 리뷰 반영)
- plans: [20260719_dedicated-tap-runloop-thread.md](20260719_dedicated-tap-runloop-thread.md) — PR #13 리뷰에서 승격된 별도 플랜(전용 런루프 스레드 재검토)
