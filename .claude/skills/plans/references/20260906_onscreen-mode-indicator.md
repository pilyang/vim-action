...
- [ ] **PR 1 — 오버레이 기반 + 전환 시 순간 표시 (요소 앵커).** 산출물: (a) 비활성화 `NSPanel` 래퍼 컴포넌트(`@MainActor`, 라벨·색·페이드 1초, 절대 `makeKey` 계열 호출 없음), (b) 순수 함수 계층 — 앵커 사다리 선택·배지 배치·화면 클램프·AX→AppKit 좌표 변환 + XCTest, (c) 기하 읽기 협력자 — 리졸버 pid로 포커스 요소 rect·포커스 창 rect를 `readQueue`/게시 큐에서 50ms로 읽어 rect만 메인으로 전달, (d) 트리거 — `EventTapController.mode` 변경 시 1회 읽기 → 순간 표시, `MenuBarIndicator.resolve`가 `.mode`일 때만. 검증: 유닛 테스트(사다리·클램프·좌표 변환, 사다리 상태별 숨김) + 도그푸딩(Notes·Slack·Chrome, 보조 디스플레이, 전체화면 앱, 최전면 앱 불변). 이 PR과 함께 architecture 신규 reference 추가.
...
- architecture: [app-shell.md](../../architecture/references/app-shell.md), [focus-and-dispatch-reads.md](../../architecture/references/focus-and-dispatch-reads.md), [reentrancy-and-safety.md](../../architecture/references/reentrancy-and-safety.md)
