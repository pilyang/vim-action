...
- **생성일**: 2026-09-06
- **갱신일**: 2026-09-15 (Settings 창 Indicator 탭 분리를 PR #68에 추가 — 이전: PR 3 확장 커밋·스크립트 도그푸딩 완료, Settings UI 경로·창 테두리 엣지는 사용자 직접 도그푸딩으로 인계)
...
- [ ] **사용자 직접 도그푸딩(2026-09-15 인계, `/Applications`에 PR 3 확장 빌드 설치됨, 설정은 제품 기본값으로 리셋)** — Settings UI 경로: "Show" Picker 전환(상시 표시 즉시 숨김·앱 복귀 시 재표시, 스타일 Picker 비활성), 스타일 Picker 3종 전환(설정 창 열린 채 이전 형태 즉시 숨김·복귀 시 새 형태 — 특히 창↔화면 테두리), 배지 떠 있는 채로 메뉴바 아이콘 반복 클릭(배지 되살아남·깜빡임 없음), Settings 창에서 Esc·i(이전 앱 배지 즉시 숨김·늦은 flash 없음). Indicator 탭 외양·탭 전환(General↔Indicator↔Apps, 창 크기 고정·Picker 비활성 표시). 창 테두리 엣지: 실제 마우스 드래그 추종(지연·잔상·CPU), 전체화면 창, 화면 밖으로 일부 나간 창(라벨 클램프), 작은 창(계산기), 두 디스플레이에 걸친 창, 시트·팝오버 열린 상태, 라벨이 툴바를 덮는 정도, 모서리 반경 외양, Chrome·Arc 창. 관측 1건(재현 안 됨): 디스플레이 간 창 이동 직후 flash가 창 오른쪽 아래(1571,1082)에 한 번 떴음 — 재현되면 보고.
- [ ] [PR #68](https://github.com/pilyang/vim-action/pull/68) 머지 확인 (CI 3잡 required, Copilot 스레드 resolve됨) — 머지되면 `/Applications`의 도그푸딩 빌드(1.0 (1))를 정식 릴리스로 되돌릴지 결정.
- [ ] **후속 — 인디케이터 외양 커스텀(색상·투명도).** 모드 라벨 알약(flash·배지)의 **색상**과 **투명도**를 사용자가 고를 수 있게 한다. 시점은 PR 3(캐럿 위치 지정) **이후**, 또는 작업 크기가 작으면 PR 3에 **함께**. 새 항목은 설정 창의 **Indicator 탭**에 더한다([20260915_settings-window-indicator-tab.md](../../decisions/references/20260915_settings-window-indicator-tab.md)). 색상 커스텀에서도 라벨 텍스트는 항상 동반(색만으로 구분하지 않음 — PRD NFR)하고, 글씨색은 PR 2의 luminance 파생을 사용자 색에 그대로 적용한다. 설정 소유권은 UserDefaults([20260906_mode-indicator-settings-in-userdefaults.md](../../decisions/references/20260906_mode-indicator-settings-in-userdefaults.md)).
