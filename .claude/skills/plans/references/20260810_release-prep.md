...
- [ ] **1단계 — 수동 서명+공증 1회 성공**: Developer ID Application 인증서 발급 → Hardened Runtime 활성화 → Developer ID 서명 빌드 → `notarytool` 제출 → `stapler` → `spctl -a` 검증. 핵심 확인 사항: Hardened Runtime 하에서 CGEventTap/AX 정상 동작 (Accessibility는 entitlement가 아닌 TCC라 문제없을 것으로 예상, 실증 필요)
...
- 레포는 public, LICENSE 없음 (확인: 2026-08-10). 현재 pbxproj: `CODE_SIGN_STYLE = Automatic`, 팀 미지정, Hardened Runtime 미설정, entitlements 파일 없음, `MARKETING_VERSION = 1.0` 고정.
