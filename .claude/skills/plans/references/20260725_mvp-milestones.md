...
- [ ] **M1 — 출력 인프라 + 안전망**: `ActionExecutor`(모든 출력의 단일 통로) + 합성 이벤트 재진입 마커 + 별도 탭 킬스위치 단축키(기본 Ctrl-Opt-Cmd-Esc) + 최소형 예외 폭주 자동 off(1초 창 카운터, 상황 따라 유연 조정 여지). keyboard-first에선 모든 출력이 합성 이벤트라 마커가 무조건 실행보다 선행.
