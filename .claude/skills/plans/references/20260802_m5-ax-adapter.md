...
- **생성일**: 2026-08-02
- **갱신일**: 2026-08-05 (**PR-C1 작업 완료 — [PR #37](https://github.com/pilyang/vim-action/pull/37) 생성됨**. 도그푸딩 2회 완료(마지막 줄 `Vk`·`Vgg` 발견·수정 포함), 남은 것은 CI·머지뿐)
...
- **PR-C1 마감 상태** — [PR #37](https://github.com/pilyang/vim-action/pull/37) 생성 완료 (2026-08-05). **도그푸딩 2회 완료**: 체크리스트 17항목 전부 확인 — 신규 어휘 전 항목 정상, Notion 페이싱 유효(재앵커 3타 완전 반영, 접두 단축 동치), Slack 폴백 동일. 발견 1건(마지막 줄 `Vk`·`Vgg` 크로싱)은 `f13307b`로 수정 후 재검증 완료. ⑦ 상한 32는 실사용 문제 없음으로 유지(도그푸딩 조절값으로 계속 열림). 남은 것은 **CI 확인·머지**뿐이며, 머지되면 이 블록과 위 "완료된 것"의 PR-C1 항목들을 접고 PR-C2 착수 지점으로 교체한다.
  - **도그푸딩 오인 금지 (세션 2 반영)**: 읽기 성공 앱에서 —
    ...
    - Notion 빠른 연타의 낡은 읽기 → 무상태 폴백(`검증 불일치` 로그)은 세션 1의 설계된 강등 그대로 — 계속 수용. **세션 2 도그푸딩에서 재확인**: Notion `vh` 비결정성의 실패 사례는 전부 `검증 불일치 [N, N)`(빈 선택 = 진입 미반영 낡은 읽기)이었고 페이싱·접두 단축 문제가 아니었다. 성공 사례는 재앵커가 끝까지 반영됨(페이싱 유효 확인).
  - **환경 한계 (세션 2 도그푸딩 발견, PR-C1 무관·기존부터 존재)**: macOS **Unicode Hex Input** 입력 소스는 Option을 16진 입력 전용으로 예약해 **Opt+화살표 키 바인딩이 아예 없다** — 손 입력도 시스템 전역으로 안 되는 알려진 한계(TextEdit·Notes 실증, 커뮤니티 수정판 레이아웃 존재: ldanet/unicode-hex-input-fix). 우리 합성 이벤트도 같은 입력 소스를 거치므로 Opt-화살표 기반 어휘 전부(`w`·`b`·`e`·`iw`·`^`·`vb` 등, PR-B 이래)가 그 입력 소스에서 무동작. 레이아웃 가드(`hasQwertyCommandKeys`)는 문자 번역 검사라 이를 못 잡는다(Unicode Hex Input은 ASCII-capable QWERTY). **대응 계획 없음** — 주 사용자가 해당 입력 소스를 쓰지 않기로 해 코드 대응은 하지 않고, 배포 시(M6) README 등 사용자 문서에 **알려진 이슈로 기록만** 남긴다(워크어라운드: ABC/US·한글 레이아웃 사용, 또는 커뮤니티 수정판 레이아웃).
