...
- **생성일**: 2026-08-02
- **갱신일**: 2026-08-06 (PR-C2 세션 1/3 완료 — ① paste wise 정확화. 다음은 세션 2/3 — ② 스크롤 `AXVisibleCharacterRange` 정확화)
...
- **PR-C2 세션 1/3 완료** (2026-08-06, worktree `feat/m5-pr-c2`): **① paste wise 정확화** — wise 기억이 클립보드 쓰기 편집 전반으로 확장됐다(끝 개행 휴리스틱은 외부 복사 전담으로 완전 강등). `PasteWiseResolver.recordEdit(_ wise:)` 일반화 + `selectionWise` note/record, 어댑터 `contentWise` 판정(change 전 범위 charwise 포함), Visual `.selection`은 **게시 확정된 begin/switch 스트림의 세션 wise**(스킵된 `V`→`v`는 화면 그대로라 wise도 불변 — 엔진 wise가 아니라 화면 진실). 결정 2건 기록: [편집 전반 기억](../../decisions/references/20260806_paste-wise-memory-covers-all-edits.md), [세션 wise 확정 스트림 추적](../../decisions/references/20260806_selection-wise-from-confirmed-stream.md) (20260730 문서 부분 supersede). architecture `strategy-dispatch.md` 갱신 완료. 앱 테스트 **1,179건**(+7) + 엔진 81건 통과, 앱 빌드 경고 0. 델타-1 규칙·게시 확정 후 기록 계약은 불변.
...
- [ ] **PR-C2 — 혼용 2b: paste wise·스크롤 정확화** (worktree `feat/m5-pr-c2`, 세션 3개 중 1개 완료): ~~① paste wise~~(세션 1 완료 — 위 "완료된 것"). 남은 것: ② 스크롤 근사 줄 수 → `AXVisibleCharacterRange` 뷰포트 정확화, ③ 비-QWERTY 정식 해소(문자→키코드 역조회 주입 — [레이아웃 가드 결정](../../decisions/references/20260801_non-qwerty-command-key-layout-guard.md)의 이연 항목). ②·③ 후 PR로 묶는다(세션 1 커밋은 로컬에만 있음 — 푸시·PR 미생성).
...
- **PR-C2 세션 2/3 착수 지점 (세션 1 인계)** — 같은 worktree(`feat/m5-pr-c2`)에서 이어간다. 스코프는 **② 스크롤 정확화만**: 현재 `CommandKeyMapper`의 스크롤은 뷰포트 높이를 모르는 근사 줄 수(`↓`/`↑` 반복, half 15·full 30, M4 프로파일 조절값)다 — `AXVisibleCharacterRange`로 뷰포트를 읽어 정확화한다. 읽기 소비 선례는 편집(범위 술어)·Visual(세션 술어) 두 곳 — 스크롤은 셋째 소비자가 된다. 주의: `FocusedText` 프리미티브는 캐럿 ±256 창이라 `AXVisibleCharacterRange`는 **새 프리미티브**다(`AXRead` 소유 규칙·50ms 타임아웃·게시 큐 위 lazy 계약을 따를 것). `.scroll`은 걸러내기 표에서 모션 편(`.nonText`도 게시)임을 유지해야 한다.
  - **세션 1이 만든 재료**: `PasteWiseResolver`가 wise 기억 일반화(`recordEdit`/`noteSelectionWise`/`recordSelectionEdit`) — 붙여넣기 쪽은 이제 완결이라 ②에서 건드릴 일 없음. "게시 확정(`.groups`) 후 상태 갱신" 계약의 세 번째 사례가 늘었다(앵커 상태·wise 기억·세션 wise note).
  ...
  - **⑦ 상한 32**(`reselectSpanClamp`)·**페이싱 5ms**(`pacedStrokeInterval`)는 도그푸딩 조절값으로 계속 열려 있다 — 실사용 문제 발견 시 그 자리에서 조정.
