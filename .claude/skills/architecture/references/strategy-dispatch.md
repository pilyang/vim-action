...
- **Last updated**: 2026-08-02 (M5 PR-B 세션 1 반영 — 읽기의 첫 소비자 배선, 0폭 포화 편집 억제로 수용 엣지 5 해소)
...
**읽은 것에서 사실을 뽑는 파생 질의는 `FocusedText`의 extension**(`FocusedTextAnalysis.swift`)이며 읽기와 파일이 나뉜다 — 소비자가 늘어나는 쪽이 여기다. 절대↔창 내 상대 오프셋 변환, 문서 시작/끝, 캐럿 기준 줄 시작/끝(개행 위치) 다섯이고, **증명하지 못하면 전부 `false`** 다(= 정확화하지 않음 = 현행 시퀀스). 오프셋 단위는 **UTF-16**이다 — `Character` 단위로 인덱싱하면 이모지가 있는 창에서 개행 위치가 밀려 "잘못 정확화"라는 안전하지 않은 방향으로 틀린다. `isAtDocumentEnd`는 `>=`가 아니라 `==`이며 창이 문서 끝에 닿았다는 방증까지 요구한다(`AXNumberOfCharacters`를 어긋나게 보고하는 앱에서 `>=`는 편집 키를 영구히 삼킨다).

**소비 지점은 `mapping`의 `.edit` 분기 한 곳**이고, 매퍼가 `EditKeyMapper.consultsFocusedText(_:)`로 "이 범위가 묻는가"를 답해 어댑터가 읽을지 정한다 — 범위 표가 어댑터로 복사되지 않고, 묻지 않는 어휘(모션·Visual·paste·undo)는 AX 왕복이 0건이다. `keyStrokes`의 시그니처는 바뀌지 않았다: 읽기를 `keyStrokes`에 넣으면 `nil`이 "미지원"과 "정확화 결과" 두 뜻을 갖게 돼 `classify`에 세 번째 프로브가 필요해지고, 무엇보다 `classify`는 `op`·`range`를 모르는 자리라 `.skipped`의 "사유를 아는 자리에서 자체 로그" 계약을 지킬 수 없다 ([20260802_read-consumption-via-mapper-predicates.md](../../decisions/references/20260802_read-consumption-via-mapper-predicates.md)).
