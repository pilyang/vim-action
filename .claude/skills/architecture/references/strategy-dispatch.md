...
- **Last updated**: 2026-08-06 (M5 PR-C2 세션 1 — paste wise 기억을 클립보드 쓰기 편집 전반으로 확장: `recordEdit(wise)` 일반화 + Visual `.selection`은 게시 확정 스트림의 세션 wise, 끝 개행 휴리스틱은 외부 복사 전담)
...
**소비 지점은 `mapping`의 `.edit` 분기와 Visual 세션 분기 두 곳**이다. 편집은 `EditKeyMapper.consultsFocusedText(_:)`로 "이 범위가 묻는가"를 답해 어댑터가 읽을지 정하는 **범위 술어**이고 — 범위 표가 어댑터로 복사되지 않는다 — Visual은 앵커 상태의 수립·검증 때문에 세션 액션마다 읽는 **세션 술어**다(M5 PR-C1, 위 Visual 매퍼 문단). 묻지 않는 어휘(모션·paste·undo)는 AX 왕복이 0건이다. 묻는 범위는 `.motion` 전체 + `.linewiseMotion(.lineUp)`·`.linewiseMotion(.documentStart)` + `.textObject(.word(.inner))`(`iw`)이며, `.line`(`dd`)과 `.linewiseMotion(.lineDown)`(`dj`)은 **일부러 빠져 있다** — 소프트 랩 논리 줄이 이 읽기로 해소되지 않아 물을 이유가 없고, 읽기 1회는 Notion에서 ~7ms이며 액션 수만큼 곱해진다.
