...
- **청크 경계는 원자 그룹 사이에만 온다** — ① 액션 1개의 전체 시퀀스(중간에서 끊으면 "선택만 만들고 `Cmd-X`는 안 나가는" 파괴적 실행), ② `.edit(_, .selection)`과 뒤따르는 `clearSelection` 사이(끊기면 살아 있는 선택이 Normal로 넘어온다), ③ `.paste`의 `접두 + 첫 Cmd-V`(접두만 나가면 붙여넣기 없이 캐럿만 움직인다). ②의 판정은 **액션 진입 시점**이어야 한다 — 처리 후에 세우면 이미 그 뒤에서 끊긴 다음이다.
