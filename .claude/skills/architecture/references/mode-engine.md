...
- **opMotion 화이트리스트**: 오퍼레이터 뒤 유효 모션은 charwise-safe 집합(`w b e h l 0 ^ $`, `.motion` 출력)과 linewise 집합(`j k G gg`, `.linewiseMotion` 출력 — 상대 j/k만 카운트 적용, 절대 G/gg는 항상 count 1)의 별도 테이블 2벌. 화이트리스트 밖은 invalid.
