...
- 탭 계층의 CGEvent→`Key` 정규화는 `KeyTranslator`(앱 타깃, `@MainActor`, keyDown 전용)가 담당한다: 특수키는 keycode, 문자키는 UCKeyTranslate + ASCII-capable 레이아웃(shift만 base에 반영, ctrl/opt/cmd는 modifiers로), 번역 불가는 `nil`이며 호출측은 무조건 통과시킨다. 아직 콜백에 배선되지 않음(다음 마일스톤) — [20260716_cgevent-key-translation-ascii-layout.md](../../decisions/references/20260716_cgevent-key-translation-ascii-layout.md).
