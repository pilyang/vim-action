...
| 앱 셸 | 메뉴바·설정 창 4탭·Dock 아이콘 수명·로그인 자동 시작·Sparkle·온보딩 push | [app-shell.md](references/app-shell.md) |
| 온스크린 모드 인디케이터 | 두 겹 오버레이 — 전환 시 캐럿 아래 flash + 비-Insert 상시 표시(배지/창 테두리/화면 테두리, 끌 수 있음), 콜백 훅·전용 읽기 큐·앵커 사다리(캐럿→요소→창)·순수 레이아웃·비활성화 패널·메뉴바 표시 사다리 재사용·자기 pid 경로·모드별 색(UserDefaults hex, 재읽기 없는 재도색)·Settings Indicator 탭 | [mode-indicator-overlay.md](references/mode-indicator-overlay.md) |
