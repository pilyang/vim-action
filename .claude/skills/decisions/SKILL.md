...
| 2026-07-30 | 스크롤은 페이지 키로 수렴 | half/full 4조합 → `PageDown`/`PageUp` 2시퀀스(half-page 프리미티브 부재, 화살표 N타는 캐럿 동반+버스트 곱으로 기각). **캐럿이 따라오지 않는 이탈** 수용 — 네이티브는 뷰만·Chromium은 캐럿도 이동해 기준 3앱에서 불일치. `Opt-Page*` A/B는 측정 이연 | [20260730_scroll-page-key-convergence.md](references/20260730_scroll-page-key-convergence.md) |
