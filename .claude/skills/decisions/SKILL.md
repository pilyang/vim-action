...
| 2026-07-26 | 모션 키스트로크 매핑 계약 | 순수 매퍼 `Motion → [KeyStroke]` — 배열 반환이 계약(모션 1개=키 N개 조합 확장 허용), CGEvent 변환은 게시 큐 위에서. 근사 3건: w≈e(Opt-→), ^≈0(Cmd-←), a/A는 캐럿 모델 자연 소멸 — 정확 의미는 M5 AX 몫 | [20260726_motion-keystroke-mapping-contract.md](references/20260726_motion-keystroke-mapping-contract.md) |
