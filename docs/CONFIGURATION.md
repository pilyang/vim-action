...
**Which path is an app on right now?** The menu bar menu carries a `Strategy:` line for the app you were last in — `AX`, `Keyboard`, or `probing…` while `auto` has yet to decide. It shows the resolved path, so it also confirms that a `strategy:` edit took effect after **Reload Config**, and it is the quickest way to tell whether an app that swallows keys is on the Accessibility path.
