...
/// 테두리 스타일(창·화면)의 상시 표시 — 호출자가 준 frame(포커스 창 rect 또는 디스플레이의
/// `visibleFrame`)을 덮는 패널 **하나**에 강조색 테두리와 모서리 라벨 알약을 함께 그린다. 두
/// 스타일이 인스턴스 하나를 나눠 쓴다 — 어느 형태로 떠 있는지는 컨트롤러가 기억한다.
///
