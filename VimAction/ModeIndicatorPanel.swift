...
/// 같아서 값 하나로 갈라 둔다. (상시 표시를 배지로 할지 창·화면 테두리로 할지는 다른 축 —
...
/// 직접 결과다 (`20260906_mode-indicator-anchor-ladder-event-driven.md` 결정 2). 테두리
...
///
/// `private`이 아닌 것은 테두리 패널이 모서리 라벨로 같은 뷰를 쓰기 때문이다 — 알약
...
/// 나누지 않는다. 테두리 패널(`ModeIndicatorBorderPanel`)만 내용이 달라 타입이 다르지만,
...
final class ModeIndicatorPanel {
    ...
    /// 비활성화 오버레이 패널의 **유일한** 생성 지점 — 알약 둘과 테두리가 전부 여기서 받는다.
