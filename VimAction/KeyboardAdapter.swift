...
nonisolated struct KeyboardAdapter: Sendable {
    ...
    /// `9999j`·`9999p`급 버스트를 킬스위치·새 입력·토글 off가 도중에 끊을 수 있게 하는 것이
    /// 목적이고(`ExecutionAbortLatch`), 생성도 청크 단위로 미뤄져 게시 전에 이벤트 2만 개를
    ...
    /// 반면 소비하는 쪽은 대상 앱이라, 간격이 없으면 2만 개를 수십 ms에 다 넘겨버려 끊을
