...
enum SyntheticEventMarker {
    ...
    nonisolated static func mark(_ event: CGEvent) {
        event.setIntegerValueField(.eventSourceUserData, value: magic)
    }

    nonisolated static func isMarked(_ event: CGEvent) -> Bool {
...
struct ActionExecutor {
    ...
    nonisolated init(postEvent: @escaping (CGEvent) -> Void = { $0.post(tap: .cgSessionEventTap) }) {
        self.postEvent = postEvent
    }

    /// 시퀀스를 순서대로 게시한다. 마킹은 이벤트마다 게시 직전에 — 마킹되지 않은
    /// 합성 이벤트는 존재하지 않는다.
    nonisolated func post(_ events: [CGEvent]) {
        for event in events {
