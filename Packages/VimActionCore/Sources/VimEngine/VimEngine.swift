...
public struct VimEngine: Sendable {
    ...
    private mutating func step(_ key: Key) -> EngineOutput {
        ...
        // 않아 Normal이 유지된다.
        if let op = Self.lineEndShorthands[key] {
            guard current.count == nil else { return .swallow }
            return complete(op, .motion(.lineEnd, count: 1))
        }
    ...
    /// 줄 끝까지 축약 키 — `d$`/`c$`/`y$`와 동일 출력으로 즉시 완결한다.
    /// `visualOperatorKeys`와 달리 `operatorKeys`에서 파생하지 않는다: 오퍼레이터가
    /// 늘 대문자 축약을 갖는다는 보장이 없어 파생은 사실이 아닌 결합이다.
    private static let lineEndShorthands: [Key: VimAction.Operator] = [
        .char("D"): .delete,
        .char("C"): .change,
        .char("Y"): .yank,
    ]
